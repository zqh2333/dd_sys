...
### 🌍 国际网络直连版 (推荐)
```bash
command -v curl >/dev/null 2>&1 || apt-get update -y && apt-get install -y curl || yum install -y curl || apk add curl; curl -sO https://raw.githubusercontent.com/zqh2333/dd_sys/main/dd_install.sh && sh dd_install.sh
